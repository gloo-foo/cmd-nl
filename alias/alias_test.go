...
func runLines(t *testing.T, input string, opts ...any) []string {
	t.Helper()
	lines, err := testable.TestLines(nl.Nl(opts...), run.Input(input))
	if err != nil {
		t.Fatal(err)
	}
	return lines
